...
async function rakutenSearch(appId, accessKey, keyword) {
  ...
  // Gibt eine Liste von Treffern zurück (nicht nur den einen günstigsten),
  // damit der Nutzer die richtige Größe/Variante selbst auswählen kann.
  const url =
  ...
  return items.map((x) => x.Item).filter(Boolean).map((it) => ({
    name: it.itemName,
    price: it.itemPrice,
    shop: it.shopName,
    url: it.itemUrl,
...
async function runSearch(rawTerm) {
  ...
  els.moreResults.classList.add("hidden");
  ...
  // Japan-Shops: japanischen Begriff bevorzugen. Wiederverkäufer: Original.
  ...
  try {
    ...
    for (const kw of searchTerms) {
      try {
        const rs = await rakutenSearch(appId, accessKey, kw);
        all.push(...rs);
      } catch (e) {
        lastError = e;
    ...
    // Doppelte (gleiche URL) entfernen, dann nach Preis aufsteigend.
    const seen = new Set();
    const uniq = [];
    for (const it of all) {
      if (it.url && !seen.has(it.url)) { seen.add(it.url); uniq.push(it); }
    }
    ...
    showResults(uniq.slice(0, 6));
    setStatus("Achte auf Größe/Menge — der günstigste Preis ist oft die " +
      "kleinste Packung. Tippe den passenden Treffer an.");
  } catch (e) {
...
function showResults(list) {
  // Günstigster Treffer als große Karte.
  showBest(list[0]);

  // Weitere Treffer (andere Größen/Varianten) als kompakte Liste.
  els.moreList.textContent = "";
  ...
  for (const item of rest) {
    ...
    nm.className = "moreName";
    nm.textContent = item.name;
    const pr = document.createElement("div");
    pr.className = "morePrice";
    pr.textContent = yen(item.price) + " · " + item.shop;
    info.appendChild(nm);
    info.appendChild(pr);
    a.appendChild(info);
    els.moreList.appendChild(a);
  }
  els.moreResults.classList.toggle("hidden", rest.length === 0);
}
